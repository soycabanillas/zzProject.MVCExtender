...
namespace zzProject.MVCExtender.Metadata.Provider
{
    public class FluentValidationProvider : ModelValidatorProvider
    {
        ...
        public delegate ModelValidator ValidatorFactory(ModelMetadata metadata, ControllerContext context);
        ...
        public void Add(Type type, string propertyName, ValidatorFactory factory)
        {
            validators.GetOrAdd(
        ...
        public class ValidatorRegistrar<TModel>
        {
            ...
            public ValidatorRegistrar<TModel> ModelValidator(ValidatorFactory delegateModelMetadata)
            {
                provider.Add(typeof(TModel), propertyName, delegateModelMetadata);
                return this;
            }

            public ValidatorRegistrar<TModel> ModelValidator(ModelValidator validator)
            {
                provider.Add(
                    typeof(TModel),
                    propertyName,
                    (metadata, context) => validator
                );
                return this;
            }

            public ValidatorRegistrar<TModel> ModelValidator<TValidator>(Func<ModelMetadata, ControllerContext, ModelValidator> validator)
            {
                provider.Add(
                    typeof(TModel),
                    propertyName,
